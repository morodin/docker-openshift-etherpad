...
oc new-app -f https://gitlab.fokus.fraunhofer.de/carsten.tittel/etherpad/raw/master/etherpad-template.yaml -p DB_USER=ether -p DB_PASS=ether -p DB_DBID=ether -p DB_PORT=3306 -p DB_HOST=mysql -p ADMIN_PASSWORD=secret
----
